...
def main():
    ...
    for size in range(15):
        ...
        p = sub.Popen(['lp_solve.exe', '-time', '-Si', '-i', f'LPModelInst{size + 1}.lp'], text=True, stdout=sub.PIPE, stderr=sub.STDOUT, bufsize=0, universal_newlines=True)
        # output = sub.check_output(['lp_solve.exe', '-time', '-Si', '-i', f'LPModelInst{size + 1}.lp'], text = True, timeout=20)
